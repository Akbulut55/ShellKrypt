...
public sealed class AutomaticBackupCoordinator
{
    ...
    public AutomaticBackupCoordinator(
        IVaultTransferService transferService,
    ...
    public static string BuildBackupPath(string backupDirectory, string vaultPath, DateTimeOffset nowUtc)
    {
    ...
    public static int ApplyRetention(string backupDirectory, string vaultPath, int retentionCount)
    {
        ...
        var files = EnumerateAutoBackupFiles(backupDirectory, vaultPath)
    ...
    public static IReadOnlyList<string> EnumerateAutoBackupFiles(string backupDirectory, string vaultPath)
    {
    ...
    private async Task<AutomaticBackupRunResult> RunAsync(AutomaticBackupContext context, bool manual)
    {
        ...
        try
        {
            ...
            var backupPath = BuildBackupPath(directory, context.VaultPath, attemptedAt);
            var summary = await _transferService.GetExportSummaryAsync(context.VaultPath, context.VaultKey);
            await _transferService.ExportEncryptedAsync(context.VaultPath, context.VaultKey, backupPath, _sessionPassphrase);
            var verified = await _transferService.GetEncryptedImportSummaryAsync(backupPath, _sessionPassphrase);
            ...
            ApplyRetention(directory, context.VaultPath, context.Schedule.RetentionCount);
