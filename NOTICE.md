# ShellKrypt Legacy: Notice

This document records notices for the archived ShellKrypt Legacy source and for
any copying or distribution separately authorized by the copyright owner. It
does not itself grant permission to distribute the software.

## Project Notice

- Repository name: ShellKrypt Legacy.
- Historical application name: ShellKrypt.
...
- Primary license or usage status: ShellKrypt Legacy Source License 1.0;
  archived source-available commercial pre-release software.
- Legacy source:
  [https://github.com/Akbulut55/ShellKrypt-legacy](https://github.com/Akbulut55/ShellKrypt-legacy).
- Separately maintained successor project:
  [https://github.com/Akbulut55/ShellKrypt](https://github.com/Akbulut55/ShellKrypt).

## License Notice

> ShellKrypt Legacy source code is distributed under the ShellKrypt Legacy
> Source License 1.0. Personal noncommercial source builds and modifications are
...
in the archived source tree. Versions are the versions declared by the project
files. Any separately authorized package must be re-audited against its final
...
source-available license. The ShellKrypt Legacy source license met that
criterion when this archive was finalized. Any dependency upgrade, separately
authorized binary distribution, or licensing-model change requires a fresh
ImageSharp license review; otherwise a Six Labors commercial license may be
required.
...
`THIRD-PARTY-NOTICES.txt` files. Any separately authorized binary package must
reproduce those package-supplied license and third-party notice files for the
platforms it actually contains.

The ShellKrypt logo and `main-logo.ico` are treated as first-party legacy assets
and are covered by the ShellKrypt Legacy Source License.
...
- Any separately authorized release must generate and verify a complete license
  bundle from the exact published artifacts; this source-tree inventory is not
  a substitute for inspecting self-contained runtime files, installers, or
  store packages.

## Archive And Distribution Status

- Archive status: development ended at version `0.28.3`.
- Official legacy distribution locations: none.
- Official signing or verification: none.
- Support or maintenance status: archived and unmaintained; no updates,
  security fixes, compatibility work, or support are promised.

Distribution rules:

- Do not present third-party, modified, or unsigned builds as tested, signed,
  maintained, endorsed, or supported ShellKrypt Legacy releases.
...
- Personal noncommercial modifications are permitted by the legacy license.
...
- Use the ShellKrypt and ShellKrypt Legacy names honestly and in a way that does
  not confuse this archived implementation with the separately maintained
  ShellKrypt project.
- Do not use project logos, screenshots, domains, or branding to imply endorsement or support without permission.
- The successor project's name, source, releases, support, and licensing are
  independent of this archive.
