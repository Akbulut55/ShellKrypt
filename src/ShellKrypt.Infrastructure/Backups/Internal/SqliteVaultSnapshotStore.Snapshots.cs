...
internal sealed partial class SqliteVaultSnapshotStore
{
    private async Task<VaultSnapshot> BuildSnapshotAsync(string vaultPath, byte[] vaultKey, CancellationToken ct)
    {
        ...
        return new VaultSnapshot(SqliteVaultSnapshotStore.CurrentVersion, DateTimeOffset.UtcNow.ToString("O"), items, snapshotLabels, itemLabels);
    }

    private static VaultSnapshotSummary SummarizeSnapshot(VaultSnapshot snapshot)
    {
